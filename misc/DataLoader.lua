...
--  print('DataLoader loading json file: ', opt.json_file)
 ...
 -- print('vocab size is ' .. self.vocab_size)
  read_list = utils.read_json('data_name_list.json')
 -- print('/'..string.sub(read_list[1],1,read_list[1]:len()-1))
  -- open the hdf5 file
  self.h5_file = hdf5.open(opt.h5_file, 'r')
  assert(#read_list > 0, 'dataset_name_list is not loaded') 
  -- extract image size from dataset
  local images_size = self.h5_file:read('/'..string.sub(read_list[1],1,read_list[1]:len()-1)):dataspaceSize()
 -- print('eric is asking..')
  local factory = self.h5_file:read('/'..string.sub(read_list[1],1,read_list[1]:len()-1)):getTensorFactory()
 -- print('video_sizes:')
  --print(images_size)
  assert(#images_size == 4, '/images should be a 4D tensor')
  assert(images_size[3] == images_size[4], 'width and height must match')
  self.num_images = #read_list
...
function DataLoader:getBatch(opt)
  ...
  local img_batch_raw = torch.ByteTensor( frames_per_video, batch_size, 3, 256, 256)
  ...
  for i=1,batch_size do
    ...
    ix = split_ix[ri]
    local block_size = self.h5_file:read('/'..string.sub(read_list[ix],1,read_list[ix]:len()-1)):dataspaceSize()
    rand_start = math.random(1, block_size[1]-9 )
    assert(ix ~= nil, 'bug: split ' .. split .. ' was accessed out of bounds with ' .. ri)
    -- fetch the image from h5
   -- print('start reading from'..'/'..string.sub(read_list[ix],1,read_list[ix]:len()-1) )
   -- print('with dataset size: ')
   -- print(block_size)
   -- print('frames from:'..rand_start)
   -- print('to '..rand_start+9)
    local img = self.h5_file:read('/'..string.sub(read_list[ix],1,read_list[ix]:len()-1)):partial({rand_start,9+rand_start},{1,self.num_channels},
                            {1,self.max_image_size},{1,self.max_image_size})
   -- print('output size: ')
    --print(img:size())
    img_batch_raw[{{}, {i}, {}, {}}] = img
    --print(img_batch_raw:size())
    --swap RGB ???
