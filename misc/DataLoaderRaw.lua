...
function DataLoaderRaw:__init(opt)
  ...
  print('DataLoaderRaw loading A video  from folder: ', opt.folder_path)
  self.prefix = utils.getopt(opt,'video_prefix','')
  self.files = {}
  self.ids = {}
  if string.len(coco_json) > 0 then
    ...
    rand_start = math.random(1, counter - 9)
    for i = rand_start, rand_start + 9 do
        tmp_file = opt.prefix ..'_'..string.format("%06d",i )..'.JPG'
        local fullpath = path.join(opt.folder_path, tmp_file)
  ...
  self.N = #self.files
  --print('DataLoaderRaw found ' .. self.N .. ' frames in the given video')
...
function DataLoaderRaw:getBatch(opt)
  ...
  local img_batch_raw = torch.ByteTensor(10, batch_size, 3, 256, 256)
  ...
  for i=1, 10 do
    ...
    -- load the image
    print('loading....'..self.files[ri])
    local img = image.load(self.files[ri], 3, 'byte')
    img_batch_raw[{ {i}, {1}, {}, {}, {} }] = image.scale(img, 256, 256)
