...
-- Data input settings
cmd:option('-input_h5','full_test_data_real.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','full_test_data.json','path to the json file containing additional info and vocab')
...
cmd:option('-input_image_encoding_size',1024,'the encoding size of each frame of image.')
cmd:option('-input_word_encoding_size',512,'the encoding size of each token in the vocabulary')
cmd:option('-word_input_layer',3, 'before which layer of LSTMs do we input the word embedding?')
cmd:option('-num_layers', 4, 'number of layers of LSTM')

-- Optimization: General
cmd:option('-max_iters', -1, 'max number of iterations to run for (-1 = run forever)')
cmd:option('-batch_size',24,'what is the batch size in number of images per batch? (there will be x seq_per_img sentences)')
...
cmd:option('-seq_per_img',10,'number of captions to sample for each image during training. Done for efficiency since CNN forward pass is expensive. E.g. coco has 5 sents/image')
...
if string.len(opt.start_from) > 0 then
  ...
  protos.crit = nn.LanguageModelCriterion(opt.frame_length) -- not in checkpoints, create manually
  ...
  lmOpt.caption_size = opt.batch_size * opt.seq_per_img -- not used
  protos.lm = nn.LanguageModel(lmOpt)
  -- initialize the ConvNet
  local cnn_backend = opt.backend
  if opt.gpuid == -1 then cnn_backend = 'nn' end -- override to nn if gpu is disabled
  local cnn_raw = loadcaffe.load(opt.cnn_proto, opt.cnn_model, cnn_backend)
  protos.cnn = net_utils.build_cnn(cnn_raw, {encoding_size = opt.input_image_encoding_size, backend = cnn_backend})
  ...
  protos.crit = nn.LanguageModelCriterion(opt.frame_length)
end
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    if verbose then
      print(string.format('outputing validation set results... %d/%d (%f)', ix0-1, ix1, loss))
    end
  ...
  if opt.language_eval == 1 then
    lang_stats = net_utils.language_eval(predictions, opt.id) -- predictions: # 224; 
  end
...
local function lossFun()
  ...
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
      ...
      for t = 1, opt.frame_length do
        local dx = protos.cnn:backward(data.images[t], dfeats[t])
      end 
  end
